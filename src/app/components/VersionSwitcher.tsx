...
export function VersionSwitcher({ value, allowed, hasAny, onChange, onAny }: Props) {
  ...
  const versions = useMemo(() => {
    if (allowed) {
      return allowed.map((a) => config.versions.find((v) => v.id === a)!).filter((v) => v !== undefined);
    }
  ...
  return (
    <BtnMenu
      class={`version-switcher${Store.getVersion() === null ? " attention" : ""}`}
      icon="tag"
      label={value ?? locale("any_version")}
      tooltip={locale("switch_version")}
    >
      {hasAny && (
        <Btn
          label={locale("any_version")}
          onClick={onAny}
          active={!value}
        />
      )}
