...
export function FileCreation({ docAndNode, gen, method }: Props) {
  ...
  const doSave = useCallback(() => {
    ...
    const id = Identifier.parse(
      fileId.includes(":") || project.namespace === undefined ? fileId : `${project.namespace}:${fileId}`,
    );
  ...
  return (
    <Modal class="file-modal">
      ...
      <TextInput
        autofocus={gen.id !== "pack_mcmeta"}
        class="btn btn-input"
        value={fileId}
        onChange={changeFileId}
        onEnter={doSave}
        onCancel={hideModal}
        placeholder={locale("resource_location")}
        spellcheck={false}
        readOnly={gen.id === "pack_mcmeta"}
      />
